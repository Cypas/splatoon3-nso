...
from .utils import DICT_RANK_POINT, get_battle_true_id, get_game_sp_id_and_name
...
async def get_evaluate_text(is_battle, detail):
    ...
    async def get_p_data(p: dict) -> dict:
        """对每个成员进行分析，取成员的数据字典
        """
        d = {
            "is_gold_x": False,  # 金X
    ...
    def analyze_team_data(team_data):
        ...
        for player in team_data:
            ...
            if hasattr(player, 'is_gold_x') and player.is_gold_x:
                has_gold_x = True
            # 检查是否有银X (假设数据中有is_silver_x字段或类似标识)
            if hasattr(player, 'is_silver_x') and player.is_silver_x:
                has_silver_x = True

            # 计算并更新最高KD (假设数据中有kill和death字段)
            if hasattr(player, 'kill') and hasattr(player, 'death'):
                if player.death > 0:
                    kd = player.kill / player.death
                    if kd > max_kd:
                        max_kd = kd

            # 更新最高击杀数
            if hasattr(player, 'kill') and player.kill > max_kill:
                max_kill = player.kill
    ...
    my_score = detail.get("myTeam").get("result").get("score")  # 我方比分
    other_score = detail.get("otherTeams")[0].get("result").get("score")  # 对方比分
    # 自己的武器名得遍历出自己数据
    my_team_players_list = detail.get("myTeam").get("players")
    other_team_players = detail.get("otherTeams").get("players")

    my = next((p for p in my_team_players_list if p.get("isMyself")), None)
    my_index = next((i for i, p in enumerate(my_team_players_list) if p.get("isMyself")), None)
    my_team_players = [p for p in my_team_players_list if not p.get("isMyself")]
    my_weapon_name = my.get("weapon").get("name")  # 武器名
    my_kd = get_kd(my)
    my_d = my['result']['death']
    ...
    # 队友和对手成绩计算
    my_team_data = [await get_p_data(p) for p in my_team_players]
